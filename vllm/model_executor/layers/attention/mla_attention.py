...
class MLACommonImpl(MLACommonBaseImpl[M], Generic[M]):
    ...
    def forward(
        ...
        # faster flashinfer RoPE, and allows fused RoPE+quant for decode.
        ...
        if has_decode:
            ...
            if (
                ...
                # Store decode k to cache (FP8 input supported by concat_and_cache_mla)
                if kv_cache.numel() > 0:
                    ops.concat_and_cache_mla(
                        decode_k_c_normed,
                        decode_k_pe,
                ...
                # Store decode k to cache (after RoPE applied)
                if kv_cache.numel() > 0:
                    ops.concat_and_cache_mla(
                        decode_k_c_normed,
                        decode_k_pe.squeeze(1),
                ...
                # Non-FP8 path: apply RoPE to Q and K
                if self.rotary_emb is not None and decode_positions is not None:
                    decode_q_pe, decode_k_pe = self.rotary_emb(
                        decode_positions,
                        decode_q_pe,
            ...
            # Convert kv_cache to fp8 view for decode attention
            if fp8_attention:
